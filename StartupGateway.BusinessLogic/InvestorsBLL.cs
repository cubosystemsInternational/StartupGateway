...
namespace StartupGateway.BusinessLogic
{
    ...
    public class InvestorsBLL
    {
        ...
        private readonly ILogger<InvestorsBLL> _logger;

        /// <summary>
        /// Constructor to initialize InvestorBLL with necessary dependencies.
        /// </summary>
        public InvestorsBLL(IUnitOfWork unitOfWork, ILogger<InvestorsBLL> logger)
        {
        ...
        public Investors GetInvestorById(int investorId)
        {
            try
            {
                using var investorRepository = _unitOfWork.GetDAL<IInvestorsDAL>();
        ...
        public bool AddInvestor(Investors investor)
        {
            try
            {
                using var investorRepository = _unitOfWork.GetDAL<IInvestorsDAL>();
                ...
                _unitOfWork.Commit();
                _logger.LogInformation("Investor added successfully: {InvestorId}.", investor.Id);
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred while adding investor: {InvestorId}.", investor.Id);
                throw;
            }
        }
        ...
        public object UpdateInvestor(Investors updatedInvestor)
        {
            try
            {
                using var investorRepository = _unitOfWork.GetDAL<IInvestorsDAL>();
                var existingInvestor = investorRepository.GetEntityById(updatedInvestor.Id);
                ...
                if (existingInvestor != null)
                {
                    ...
                    _unitOfWork.Commit();
                    _logger.LogInformation("Investor Updated successfully: {InvestorId}.", existingInvestor.Id);
            ...
            {
                _logger.LogError(ex, "Error occurred while updating investor with ID: {InvestorId}.", updatedInvestor.Id);
                throw;
            }
        }
    }
